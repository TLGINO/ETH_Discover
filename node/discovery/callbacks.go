...
func (dn *DiscoveryNode) ExecPing(m discv4.Packet, from string) {
	...
	if err != nil {
		log.Err(err).Msg("error creating Pong")
		return
	}

	pongData, err := pongPacket.Serialize()
	if err != nil {
		log.Err(err).Msg("error serializing Pong")
		return
	}
...
func (dn *DiscoveryNode) ExecNeighbors(m discv4.Packet, from string) {
	...
	if ch != nil {
		...
		case ch <- struct{}{}:
			// Successfully sent
		default:
			log.Info().Msgf("channel full or closed for neighbour response from %s", from)
		}
	} else {
		log.Info().Msgf("unsolicited or delayed neighbour message received from %s", from)
	}
}
func (dn *DiscoveryNode) ExecENRRequest(m discv4.Packet, from string) {
	enrRequest := m.Data.(*discv4.ENRRequest)
	log.Info().Msgf("received enrRequest: %s", enrRequest)

	// [TODO] respond
}
func (dn *DiscoveryNode) ExecENRResponse(m discv4.Packet, from string) {
	enrResponse := m.Data.(*discv4.ENRResponse)
	log.Info().Msgf("received enrResponse: %s", enrResponse)
}
