...
func Init(testNode *interfaces.ENode) (*DiscoveryNode, error) {
	dn := DiscoveryNode{
		udp:      new(UDP),
		registry: &Registry{},
	...
	if testNode != nil {
		dn.AddENode(testNode)
		dn.UpdateENode(testNode, interfaces.AnsweredFindNode)
	} else {
		dn.AddENode(&e1)
		// dn.UpdateENode(&e1, interfaces.AnsweredFindNode)
		dn.AddENode(&e2)
		// dn.UpdateENode(&e2, interfaces.AnsweredFindNode)
		dn.AddENode(&e3)
		// dn.UpdateENode(&e3, interfaces.AnsweredFindNode)
		dn.AddENode(&e4)
		dn.UpdateENode(&e4, interfaces.AnsweredFindNode)
